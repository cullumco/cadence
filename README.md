...
- **Richest signals on macOS.** The deepest ambient probes — music via
  ...
  Focus/DND, focused app — read the Mac around you. On other platforms Cadence
  still runs and still moves the dials: prompt intent, self-report, git,
  time/day, and typing tempo work anywhere, Spotify can be linked for music,
  and the Mac-only probes degrade silently (CI runs the suite on Linux to keep
  that true).
- **Node 20+**
...
- **Richest signals on macOS.** The deepest ambient probes (music-via-OS,
  ...
  Other platforms keep the universal dial-movers — intent, self-report, git,
  time/day, typing tempo, linked Spotify — and the rest degrades silently.
